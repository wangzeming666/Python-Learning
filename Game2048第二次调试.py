...
# 数字
    ...
    def duiqi(self, wList):
        ...
        # 将移除的零补充在另一侧
        nones = ['' for x in range(4 - len(wList))]
...
# 引入多线程
            ...
            for col in range(4):
                ...
                wList = self.duiqi(wList)
                for i in [3, 2, 1]:
                    if wList[i] == wList[i-1] != '':
                        
                        self.fir = (col, i)
                        self.sec = (col, i-1)
                        wList = str(int(wList[i-1]) * 2)
                        self.text = wList[i-1]
                        wList[i] = ''
...
# 随机出现一个数字，画一个方块，记录位置。相加后，记录结果位置。由此得到移动先后的两个位置
    ...
    def move(self):
        ...
        self.WeiZhi(self.fir[0],self.fir[1])
        x, y = self.Heng, self.Gao
        self.WeiZhi(self.sec[0], self.sec[1])

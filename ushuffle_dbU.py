...
def connect(db):
    ...
    if db == 'sqlite':
        ...
        if not os.path.isdir(dbDir):
            os.mkdir(dbDir)
        os.sqlite.connect(os.path.join(dbDir, DBNAME))
...
getRC = lambda cur: cur.rowcount if hasattr(cur,
                                            'rowcount') else -1
def update(cur):
    fr = rand(1,5)
    to = rand(1,5)
